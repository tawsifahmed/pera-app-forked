...
<template>
    <div>
        ...
        <Dialog v-model:visible="spaceFormInputs" :style="{ width: '32rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }" header="Create Space" :modal="true" class="p-fluid">
            <div class="field">
                <label for="name">Space Name <span v-tooltip.right="{ value: 'Demo Text Text Demo Text Text Demo Text Text Demo Text Text Demo Text Text.' }" class="pi pi-question-circle cursor-pointer ml-1 text-sm"></span></label>
                <InputText id="name" v-model="spaceNameInput" required="true" :invalid="spaceNameError" />
            </div>
            <div class="field">
                <label for="name">Space Description <span v-tooltip.right="{ value: 'After setting space name, you can provide description.' }" class="pi pi-question-circle cursor-pointer ml-1 text-sm"></span></label>
                <Textarea id="description" v-model="spaceDescripInput" rows="3" cols="20" :invalid="spaceDescriptionError" />
            </div>
            <div class="field">
                <label for="name">Space Color <span v-tooltip.right="{ value: 'Demo Text Text' }" class="pi pi-question-circle cursor-pointer ml-1 text-sm"></span></label>
